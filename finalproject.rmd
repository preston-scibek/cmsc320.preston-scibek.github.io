...
ggplotly(graphed) 
```
Mookie leads the league in HRS.
...
```{r predict}
set.seed(1234)
train_indices <- sample(nrow(mookie_bat_data), nrow(mookie_bat_data)/2)
train_set <- mookie_bat_data[train_indices,]
test_set <- mookie_bat_data[-train_indices,]
...
#mookie_bat_data_converted$events = as.numeric(mookie_bat_data_converted$events)
mookie_bat_data_converted 
mookie_tree <- tree(events~pitch_name+plate_x+plate_z+release_speed, data=mookie_bat_data_converted)
plot(mookie_tree)
text(mookie_tree, pretty=0)

# Random Forest

#mookie_rf <- randomForest(events~pitch_name+plate_x+plate_z, importance=TRUE, mtry=3, data=train_set, na.action=na.roughfix)

#variable_importance <- importance(mookie_rf)
